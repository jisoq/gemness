...
The default reviewer flow uses `start_antigravity` and `await_antigravity_run`. `start_antigravity` creates an Observer run and returns immediately with `run_id`, `conversation_id`, and `observer_url`; use `mode=ask`, `mode=json`, `mode=review_current_diff`, or `mode=follow_up` to choose the run type. The reviewer should pass the parent `delegation_id` as `idempotency_key` when present, and may set `caller_role="reviewer_subagent"` so ownership is explicit in tool-call metadata. `await_antigravity_run` waits only for a short bounded interval, then returns either the final result or the current running state. Use `timeout_sec=0` to poll immediately without waiting. Non-terminal states such as `accepted`, `queued`, `sending`, `running`, `repairing`, or `result_pending=true` mean the advisory does not exist yet; the reviewer must keep polling or hand off pending state without inventing advice from progress events or elapsed time. `cancel_antigravity_run` requests cancellation by `run_id`, but should not be used merely because one await call timed out or the run is slower than expected.

The blocking final-result tools `ask_antigravity`, `follow_up_antigravity`, `ask_antigravity_json`, and `review_current_diff_with_antigravity` are compatibility wrappers. In Codex they are not the main-agent default when `codex_host.multi_agent.available` is true; use them only from a reviewer subagent, an explicit main-agent takeover with `takeover_reason`, or a non-Codex client. They return cleaned advisory text or structured data plus `observer_url`; they do not return the full raw transcript. If the host capability cache says multi-agent is available and a direct call lacks an ownership hint, Gemness adds `codex_delegation_warning` to the result.
...
The reviewer subagent spawn profile is `model="gpt-5.5"` with `reasoning_effort="medium"`. The reviewer handles Gemness orchestration and concise result synthesis while Antigravity CLI performs the primary advisory reasoning.

When the parent `delegated_run handoff` selects `mode="follow_up"` and provides the parent session id plus follow-up instruction, the reviewer is only the delivery owner for that follow-up. It must forward the instruction to Antigravity with `follow_up_antigravity` or `start_antigravity(mode="follow_up")`, wait for a terminal result, and report Antigravity's answer instead of answering the follow-up from the reviewer model's own reasoning. It must not self-initiate `follow_up_antigravity` or `mode="follow_up"` after reading an answer; if another Antigravity turn seems useful, it reports that recommendation back to the parent.

The main agent may take over only when reviewer spawn fails, the reviewer explicitly fails or times out at the subagent/task level, the reviewer returns a pending handoff or only a `run_id` without final advisory, or the user explicitly requests main-agent direct execution. A single bounded await timeout is not reviewer failure and not evidence that Antigravity will not answer. During takeover, the main agent should not start a duplicate run; it should await, cancel, or follow up using existing run or session identifiers when available, and it must not invent advisory content while the run is non-terminal.
...
- recommended Codex delegation flow under `codex_delegation`
