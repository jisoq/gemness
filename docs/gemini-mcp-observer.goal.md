...
3. UI 접속에는 random local token을 사용한다.
4. token은 MCP result의 observer_url에 포함하거나 local file에 저장한다.
...
15. token 없이 observer API에 접근할 수 없다.
