...
def test_process_registry_terminate_orphans_only_for_registered_orphans(tmp_path, monkeypatch) -> None:
    ...
    orphan = ProcessRecord(
        ...
        version="test",
        registry_id="orphan",
        process_identity={"start_marker": "same-process"},
    )
    ...
    monkeypatch.setattr("gemness.process_registry.process_identity", lambda pid: {"start_marker": "same-process"} if pid == 12345 else {})
...
def test_process_registry_refuses_orphan_termination_when_pid_identity_differs(tmp_path, monkeypatch) -> None:
    config = GemnessConfig(transcript_dir=tmp_path / "transcripts", process_registry_dir=tmp_path / "processes", observer_enabled=False)
    registry = ProcessRegistry(config)
    orphan = ProcessRecord(
        pid=12345,
        ...
        version="test",
        registry_id="orphan",
        process_identity={"start_marker": "old-process"},
    )
    (config.process_registry_dir / "12345-orphan.json").write_text(json.dumps(orphan.to_dict(), ensure_ascii=False), encoding="utf-8")
    terminated: list[int] = []

    monkeypatch.setattr("gemness.process_registry.process_is_running", lambda pid: pid == 12345)
    monkeypatch.setattr("gemness.process_registry.process_identity", lambda pid: {"start_marker": "new-process"} if pid == 12345 else {})
    monkeypatch.setattr("gemness.process_registry.terminate_process", lambda pid: terminated.append(pid))
    ...
    assert result["errors"][0]["pid"] == 12345
    assert (config.process_registry_dir / "12345-orphan.json").exists()
