...
- `use gemness` 트리거 동작을 담은 `gemness` skill을 전역 사용자 홈에 설치/갱신합니다.
...
3. `await_antigravity_run(run_id, timeout_sec=5)`처럼 짧은 대기 호출을 반복해 terminal 상태(`completed`, `valid`, `invalid`, `error`, `cancelled`)를 확인합니다. 즉시 상태만 보려면 `timeout_sec=0`을 사용합니다.
4. 필요하면 `event_cursor`를 넘겨 새 이벤트만 조회합니다.
5. `accepted`, `queued`, `sending`, `running`, `repairing`, `result_pending=true`는 pending 상태입니다. 이 상태에서는 progress/heartbeat/elapsed time으로 advisory를 추론하거나 답변을 지어내지 말고 계속 poll하거나 pending handoff만 반환합니다.
6. 중단이 필요하면 `cancel_antigravity_run(run_id)`을 호출합니다. 오래 걸리거나 단일 await가 timeout됐다는 이유만으로 취소하지 말고, 명시적인 사용자/parent 취소 요청 또는 잘못 시작한 run일 때만 취소합니다.
...
reviewer subagent는 parent의 `delegated_run handoff`가 명시적으로 `mode="follow_up"`와 parent session id, follow-up instruction을 준 경우 그 follow-up의 전달자 역할만 맡습니다. 이때 reviewer는 parent의 follow-up에 직접 답하지 않고 `follow_up_antigravity` 또는 `start_antigravity(mode="follow_up")`로 Antigravity에 전달한 뒤 terminal 결과를 기다려 Antigravity의 답을 보고합니다. reviewer가 결과를 본 뒤 스스로 더 물어보고 싶거나 추가 확인이 필요하다고 판단한 경우에는 `follow_up_antigravity`를 직접 호출하지 않고, parent에게 그 권고와 불확실성을 돌려줍니다.
