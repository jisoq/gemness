...
5. When spawning the reviewer with `multi_agent_v1.spawn_agent`, set `model="gpt-5.5"` and `reasoning_effort="medium"`. Leave other spawn profile fields unset unless the user explicitly asks for them. If the spawn call fails because of host call-shape constraints, do not fall back to main-agent Gemness tools yet: retry with a simpler valid shape such as `items` without `message`, `message` without `items`, no `fork_context` when setting model/profile fields, or no model/profile fields when a full-history fork is required. Treat direct fallback as available only after the simplified reviewer spawn also fails.
...
10. If multi-agent support is unavailable, unknown after probing, or blocked by the host, state that fallback briefly before using Gemness MCP tools directly from the main agent. Do not silently use blocking main-agent calls when the cached host capability says multi-agent is available. When direct takeover is unavoidable, include `caller_role="main_agent_takeover"` and a concrete `takeover_reason`; otherwise let the reviewer pass `caller_role="reviewer_subagent"`.
...
3. Use `ask_antigravity`, `ask_antigravity_json`, `review_current_diff_with_antigravity`, or `follow_up_antigravity` only as blocking convenience wrappers when the multi-agent reviewer flow is unavailable, the main has an explicit takeover reason, or a non-Codex client explicitly needs the compatibility wrapper. In Codex, direct wrapper calls should include `caller_role="reviewer_subagent"`, `caller_role="main_agent_takeover"` plus `takeover_reason`, or `caller_role="non_codex_client"` as applicable.
