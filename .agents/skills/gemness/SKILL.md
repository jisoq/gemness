...
5. When spawning the reviewer with `multi_agent_v1.spawn_agent`, set `model="gpt-5.4-mini"` and `reasoning_effort="high"`. Leave other spawn profile fields unset unless the user explicitly asks for them.
...
9. The main may take over a delegated Gemness run only if reviewer spawn fails, the reviewer explicitly fails or times out, the reviewer returns only a `run_id` without final advisory, or the user explicitly asks the main agent to run Gemness directly. On takeover, do not start a duplicate run; await, cancel, or follow up using the existing `run_id` or session identifiers when available.
